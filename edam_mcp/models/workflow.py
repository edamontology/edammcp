...
class WorkflowFunction(BaseModel):
    ...
    configurable_options: list[str] = Field(
        default_factory=list, description="List of configurable options/parameters"
    )
    dependencies: list[str] = Field(
        default_factory=list, description="Other workflow functions this depends on"
    )
...
class WorkflowSummaryResponse(BaseModel):
    ...
    workflow_steps: list[str] = Field(
        ..., description="Ordered list of workflow step names"
    )
    functions: list[WorkflowFunction] = Field(
        ..., description="Detailed description of all available functions"
    )
    configurable_options: dict = Field(
        ..., description="Global configurable options and their defaults"
    )
    workflow_flow: str = Field(
        ..., description="Textual description of the workflow flow"
    )
